...
local movedBuckets = NS.BuildXPGraphBuckets({ { t = 100, xp = 25 } }, 110, 60, 6)
assert_eq(25, movedBuckets[5], "graph bucket helper should move a gain left as wall-clock time advances")
assert_eq(0, movedBuckets[6], "graph bucket helper should clear the newest bucket when no current XP exists")
local retainedBuckets = NS.BuildXPGraphBuckets({ { t = 100, xp = 25 } }, 159, 60, 6)
assert_eq(25, retainedBuckets[1], "graph bucket helper should keep old XP inside the rolling window")
...
SetTime(395)
graphTicker:Fire()
assert_eq(2, frame.graphBars[12]:GetHeight(), "graph profile should clear the old bucket after idle wall-clock time advances")
assert_eq(2, frame.graphBars[18]:GetHeight(), "graph profile should clear the newest bucket when no XP happened recently")
assert_true(frame.graphBars[8]:GetHeight() > frame.graphBars[12]:GetHeight(), "older XP should slide left on heartbeat while idle")
assert_true(frame.graphBars[14]:GetHeight() > frame.graphBars[18]:GetHeight(), "newer XP should slide left on heartbeat while idle")
local stableBuckets, stablePeak = NS.BuildXPGraphBuckets(NS.state.events, 395, 60, 6)
assert_eq(100, stableBuckets[3], "graph bucket helper should slide older gains left as wall-clock time advances")
assert_eq(40, stableBuckets[5], "graph bucket helper should slide newer gains left as wall-clock time advances")
