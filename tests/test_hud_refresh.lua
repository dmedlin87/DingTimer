...
assertStringMatch("Peak bucket +100", frame.graphPeakText:GetText(), "graph profile should label the largest bucket total")
...
assert_eq("BOTTOMRIGHT", peakPoint, "graph peak label should sit above the plot area")
assert_eq(frame.graphArea, peakRelativeTo, "graph peak label should anchor to the graph area")
assert_eq("TOPRIGHT", peakRelativePoint, "graph peak label should sit above the graph top line")
assert_eq(1, peakY, "graph peak label should clear the graph top line")
...
local newestBuckets, newestPeak = NS.BuildXPGraphBuckets({ { t = 100, xp = 25 } }, 100, 60, 6)
assert_eq(25, newestBuckets[6], "graph bucket helper should place a just-earned gain in the newest wall-clock bucket")
assert_eq(25, newestPeak, "graph bucket helper should report the only visible bucket as the peak")
local idleBuckets = NS.BuildXPGraphBuckets({ { t = 100, xp = 25 } }, 110, 60, 6)
assert_eq(25, idleBuckets[6], "graph bucket helper should keep a past gain fixed as wall-clock time advances")
assert_eq(0, idleBuckets[5], "graph bucket helper should not move a past gain into older buckets on idle heartbeats")
local retainedBuckets = NS.BuildXPGraphBuckets({ { t = 100, xp = 25 } }, 159, 60, 6)
assert_eq(25, retainedBuckets[6], "graph bucket helper should keep old XP inside the rolling window without idle drift")
local expiredBuckets, expiredPeak = NS.BuildXPGraphBuckets({ { t = 100, xp = 25 } }, 161, 60, 6)
assert_eq(0, expiredBuckets[1], "graph bucket helper should drop XP outside the rolling window")
assert_eq(0, expiredPeak, "graph bucket helper should clear the peak when no XP is inside the rolling window")
local summedBuckets, summedPeak = NS.BuildXPGraphBuckets({
  { t = 103, xp = 15 },
  { t = 105, xp = 10 },
  { t = 89, xp = 70 },
}, 110, 60, 6)
assert_eq(25, summedBuckets[6], "graph bucket helper should sum multiple gains in the same wall-clock bucket")
assert_eq(70, summedPeak, "graph bucket helper should use the largest bucket total as the peak")
ClearTooltip()
...
assertStringMatch("Latest 3s", tooltipLines[3], "graph bar tooltip should show the hovered bucket time range")
assert_true(string.find(tooltipLines[3], "before latest gain", 1, true) == nil, "graph bar tooltip should use wall-clock wording")
assertStringMatch("Peak bucket +100", tooltipLines[4], "graph bar tooltip should show the peak bucket for context")
...
SetTime(395)
graphTicker:Fire()
assert_true(frame.graphBars[12]:GetHeight() > frame.graphBars[18]:GetHeight(), "older larger gains should stay fixed while idle")
assert_true(frame.graphBars[18]:GetHeight() >= 3, "newer gains should stay fixed while idle")
assert_eq(2, frame.graphBars[8]:GetHeight(), "idle heartbeat should not move older XP into a different bar")
assert_eq(2, frame.graphBars[14]:GetHeight(), "idle heartbeat should not move newer XP into a different bar")
local stableBuckets, stablePeak = NS.BuildXPGraphBuckets(NS.state.events, 395, 60, 6)
assert_eq(100, stableBuckets[4], "graph bucket helper should keep older gains fixed as wall-clock time advances")
assert_eq(40, stableBuckets[6], "graph bucket helper should keep newer gains fixed as wall-clock time advances")
assert_eq(100, stablePeak, "graph bucket helper should preserve the peak bucket total while XP remains visible")
...
assert_eq("No gold in 60s", frame.titleText:GetText(), "auto mode should switch to gold text at max level")
assert_false(frame.progressBar:IsShown(), "max-level auto gold mode should hide the XP progress bar")
assert_false(string.find(frame.titleText:GetText(), "to level", 1, true) ~= nil, "max-level gold title should not mention leveling")
assert_false(string.find(frame.subText:GetText(), "Need", 1, true) ~= nil, "max-level gold detail should not show remaining XP")
...
assertStringMatch("Session +1|cffffd700g|r", frame.subText:GetText(), "gold mode should show net session money")
assert_false(frame.progressBar:IsShown(), "gold mode should keep the XP progress bar hidden after money gains")
...
NS.SetHUDProfile("full")
NS.RefreshFloatingHUD(510)
assert_eq("Max level", frame.titleText:GetText(), "manual XP mode at max level should show a max-level title")
assert_false(string.find(frame.subText:GetText(), "Need", 1, true) ~= nil, "manual XP mode at max level should not show remaining XP")
assert_false(frame.progressBar:IsShown(), "manual XP mode at max level should not show a capped XP bar")
